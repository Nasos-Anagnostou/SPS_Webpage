...
if not df.empty:
    ...
    if "MBA" in str(df['MAGNET_MEASURED']):
        ...
        elif "Other" in str(df['FLUXMETER_MEASURED']):
    ...
    elif "MBB" in str(df['MAGNET_MEASURED']):
        st.session_state.magnettype = "Dipole"
        ...
        if "B5" in str(df['FLUXMETER_MEASURED']):
    ...
    elif "Quadrupole" in str(df['MAGNET_MEASURED']):
        st.session_state.magnettype = "Quadrupole"

        if "Q4" in str(df['FLUXMETER_REFERENCE']):
        ...
        if "Q2" in str(df['FLUXMETER_MEASURED']):
    ...
    if input_method == "Choose a specific Workorder and Date":  # 1st option
        ...
        if not mydf.empty:
            ...
            mydf = mydf.rename(columns={'CURRENT_APPLIED':'Current'})
            # Show the average and sttdev tables
            avg_stddev_tables(mydf,coils_used)
    ...
    # Display all data in a Streamlit table and apply filters
        ...
        if st.button("Analyse Measurement"):
            #st.session_state.flag = True
            # Save the modified DataFrame to a file (modify this part based on your saving preference)
            df_modified = pd.DataFrame(mydf)
            df_modified = df_modified.iloc[:, 6:]
            make_df(df_modified, False)
